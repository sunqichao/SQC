...
-(id)initWithFrame:(CGRect)frame{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        //添加各种通知
        [self addHidenLoginViewNotification];
        
        [self addHidenRegisterViewNotification];
...
-(void)initializeViewComponents{
    ...
    self.BackgroundImageView.contentMode = UIViewContentModeScaleAspectFill;//可能会有部分图片显示
    self.BackgroundImageView.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleHeight;//自适应
    ...
    //Page Control
    self.PageControl = [[UIPageControl alloc] initWithFrame:CGRectMake((self.frame.size.width - kPageControlWidth)/2, self.frame.size.height - 48 -44, kPageControlWidth, 37)];
    ...
    UIImageView *loginBar = [[UIImageView alloc] initWithFrame:CGRectMake(0, self.frame.size.height-44, 320, 44)];
    loginBar.image = [UIImage imageNamed:@"LoginAndRegister.png"];
    [self addSubview:loginBar];
    
    self.loginButton = [UIButton buttonWithType:UIButtonTypeCustom];
    self.loginButton.frame = CGRectMake(25, 527, 128, 37);
    ...
    self.registerButton.frame = CGRectMake(172, 527, 128, 37);
    [self.registerButton addTarget:self action:@selector(registerMethod:) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:self.registerButton];
    
    self.loginController = [[SQCLoginViewController alloc] init];
    
    self.registerController = [[SQCRegisterViewController alloc] init];
...
- (void)loginMethod:(id)sender
{
    ...
    [UIView setAnimationDuration:DismissLoginAndRegisterTime];
    [UIView setAnimationDelegate:self];
    
    [UIView setAnimationTransition:UIViewAnimationTransitionCurlDown forView:self cache:YES];
    
    [self addSubview:self.loginController.view];
    ...
    [NSNotificationCenter.defaultCenter addObserverForName:@"AppearRegisterViewNotification"
                                                ...
                                                usingBlock:^(NSNotification *note)
     {
         ...
         dispatch_async(dispatch_get_main_queue(), ^{
             ...
             [self addSubview:self.registerController.view];
...
- (void)addHidenRegisterViewNotification
{
    [NSNotificationCenter.defaultCenter addObserverForName:@"HidenRegisterViewNotification"
                                                    object:nil
                                                     queue:nil
                                                usingBlock:^(NSNotification *note)
     {
         NSLog(@"HidenRegisterViewNotification ********");
...
- (void)addHidenLoginViewNotification
{
    [NSNotificationCenter.defaultCenter addObserverForName:@"HidenLoginViewNotification"
                                                    object:nil
                                                     queue:nil
                                                usingBlock:^(NSNotification *note)
     {
         NSLog(@"HidenLoginViewNotification ********");
